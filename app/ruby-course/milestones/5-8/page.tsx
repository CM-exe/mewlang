    ...
    <div className="theme-ruby">
      <div className="wrap">
        <header className="masthead">
          ...
          <p className="lede">The AST gets an interpreter with middleware, failures become a design rather than a rescue, third parties get to add verbs, and the steps start talking to a real socket and a real store.</p>
        </header>
        <div className="note">
          <h5>Verification note</h5>
          <p>All code was run on Ruby 3.2.3. The suite is now 28 tests and 74 assertions, all passing, including tests that start a real HTTP server on a real socket. Three of the bugs described below are ones I actually hit while writing this, kept because each one teaches something the working code cannot.</p>
        </div>
        ...
        <p><strong>1. What flows between steps?</strong> Milestone 2 passed the bare return value, which meant a step could not know the pipeline's name, whether this was a dry run, or what earlier steps had learned. So we introduce a <code>Context</code>: payload plus run metadata. It is immutable, and a step "changes" it by producing a new one.</p>
        <p><strong>2. Does a failure raise or return?</strong> Both, but the default is to return. A <code>RunResult</code> that says <code>ok: false</code> and carries every step's outcome is much more useful to a CLI, a dashboard or a test than an exception, because a failed run still has partial results worth showing. <code>run!</code> exists for callers who prefer the exception.</p>
        <p><strong>3. Where do logging, timing and dry runs live?</strong> Not in the runner. Each is a <em>middleware</em>: a callable wrapping each step, exactly like Rack. This is the difference between a runner you can extend and one you have to edit.</p>
        ...
        <ul>
          ...
          <li><code>dry_run?</code> exists because <code>Data</code> generates <code>dry_run</code> but Ruby convention wants a question mark on a boolean reader. Adding the alias costs one line and makes every call site read properly.</li>
        </ul>
        ...
        <p><code>Middleware.build</code> is four lines and repays reading slowly, because the same fold appears in Rack, in Rails' middleware stack, in Plug, and in Express.</p>
        <ol>
          ...
          <li>For each layer, produce a new two-argument lambda that calls that layer, handing it the chain built so far as its <code>nxt</code>.</li>
          <li>The final value is a single callable that, when invoked, threads through every layer in the original order.</li>
        </ol>
        <p>The reversal is the part people get wrong. Folding forwards would run the list inside-out, so your logging middleware would report after the step instead of before it. If you ever need to convince yourself, build the chain with two layers that print on the way in and on the way out, and watch the nesting.</p>
        <p>Each middleware is a plain <code>lambda</code>, not a class, because the contract is one method. A lambda is also strict about arity, so passing a two-argument one is an immediate <code>ArgumentError</code> rather than a confusing <code>nil</code>.</p>
        ...
        <ul>
          ...
          <li><code>e.is_a?(Error) ? e : StepFailed.new(step, e)</code> — our own errors pass through unwrapped, foreign errors get wrapped with the step that caused them. We will revisit whether that is the right call in Milestone 8, where it produces a mildly surprising result in a test.</li>
        </ul>
        ...
        <p>Twelve lines instead of a dependency, and the important one is <code>Log.null</code>: an object that satisfies the interface and does nothing. <strong>The null object pattern is how you avoid <code>if @log</code> scattered through a codebase</strong>, and it is why the runner can call <code>context.log.info</code> unconditionally. A library that forces a logging framework on its users is a library people wrap to make it quiet.</p>
        <h3>Running it</h3>
        ...
        <p>The log lines carry <code>demo5.rb:10</code>, the user's own file and line, which is the Milestone 4 source locations paying a second dividend. When a pipeline misbehaves in production, the log points at the line that wrote the step.</p>
        <div className="exercise">
          <h5>Exercise 5</h5>
          <p>Write a <code>Memoize</code> middleware that skips a step when it has already been run with the same input during this process, returning the cached value. Then add <code>Runner#use(middleware)</code> so users can add their own.</p>
          <p>Requirements: the cache key must include the step name, its options and the input; a dry run must never populate the cache; the cache must be per-Runner, not global; and there must be a way to see cache hits in the log. Write a test proving a second run does not call the underlying step.</p>
          <p>Think carefully about the key. What happens if the input is a 50,000-element array, or an object that does not define <code>hash</code>?</p>
        </div>
        <details>
          ...
          <ul>
            <li><strong>A middleware can be an object, not just a lambda.</strong> Anything with <code>#call(step, context, nxt)</code> works, and an object is what you need when the middleware has state. The duck-typed contract pays off again.</li>
            <li><strong>Keys must not hold the payload alive.</strong> Using the array itself as a Hash key keeps every input in memory for the process lifetime, which is a memory leak wearing a cache costume. Hashing the content fixes the retention; note that it also costs a full serialisation, so for large inputs the "optimisation" may be slower than the step.</li>
            <li><strong><code>:uncacheable</code> as a fallback is the important line.</strong> <code>Marshal.dump</code> raises on procs, IO objects and anything with a singleton class, which in this project includes a payload containing a <code>Store</code>. Rescuing and declining to cache is correct; rescuing and using <code>object_id</code> as the key would produce a cache that returns the wrong answer, which is much worse than no cache.</li>
            <li><strong><code>use</code> returns <code>self</code> and rebuilds the array rather than mutating</strong> the frozen default constant. <code>DEFAULT_MIDDLEWARE</code> is frozen precisely so this mistake raises instead of corrupting every future Runner.</li>
          </ul>
          <p>One honest caveat on the design: memoizing steps assumes they are pure, and <code>save_to</code> certainly is not. A production version would let a step declare <code>cacheable false</code>, which is another thing the plugin metadata of Milestone 7 makes easy.</p>
        </details>
        <h4>Experiment</h4>
        <p>Reverse the middleware order in <code>DEFAULT_MIDDLEWARE</code> so it reads <code>[Timing, DryRun, Logging]</code> and run a dry run. Timing now wraps the dry-run short circuit, so you get timings for steps that never ran, and the logging line never appears at all because <code>DryRun</code> returns before reaching it. Middleware order is semantics, not style, and this is the cheapest possible way to feel that.</p>
        <div className="warn">
          <h5>Common mistakes in Milestone 5</h5>
          <ul>
            <li><strong>Folding the middleware list forwards.</strong> The chain runs inside-out and nothing behaves as written.</li>
            <li><strong>Forgetting to call <code>nxt</code></strong> in a middleware that was supposed to be transparent. The step silently never runs, and the pipeline still reports success.</li>
            <li><strong>Mutating the context</strong> instead of using <code>with</code>. With <code>Data</code> you get a <code>FrozenError</code>; with a plain class you get action at a distance.</li>
            <li><strong>Using <code>Time.now</code> for durations.</strong> Use the monotonic clock.</li>
            <li><strong>Building the chain inside the step loop.</strong> Works, wastes time proportional to steps × middleware.</li>
            <li><strong>Taking a logging gem as a dependency.</strong> A twelve-line null-object logger and a documented interface serve your users better.</li>
          </ul>
        </div>
        <h4>Checkpoint</h4>
        <ol>
          ...
          <li>Why does <code>run</code> return a result rather than raising, and when would you want <code>run!</code>?</li>
        ...
        <h3>Concepts</h3>
        <p>Retry as middleware, policies as data, exception chaining and the limits of <code>cause</code>, and handlers that must not be able to break the run.</p>
        <h3>Design</h3>
        ...
        <ul>
          ...
          <li><code>base_delay</code> is configurable mainly so tests can use 10 ms instead of a second. A retry policy you cannot speed up is a retry policy your test suite will not exercise.</li>
        </ul>
        ...
        <p><code>rescue *policy.errors</code> splats an array of exception classes into the rescue clause, which is how you make the caught set dynamic. <code>retry</code> restarts the <code>begin</code> block, so no loop is needed.</p>
        <div className="note">
          <h5>The one mutable object in the system</h5>
          <p>The <code>counter</code> exists because the middleware knows how many attempts happened and the runner is the one building the <code>StepResult</code>. Everything else here is immutable, and threading the count back through a frozen context is impossible by construction.</p>
          <pre className="plain"><code>{"class AttemptCounter\n  attr_accessor :count\n  def initialize = @count = 1\n  def reset = @count = 1\nend"}</code></pre>
          <p>When an immutable design needs one channel for information flowing backwards, the honest move is a small, explicitly scoped mutable object with a name that says what it is, rather than making the whole context mutable "just in case". It is reset before each step and read immediately after, so its lifetime is one step.</p>
        </div>
        ...
        <div className="warn">
          ...
          <p>The handler is called with <code>error.cause || error</code>, and <code>cause</code> was <code>nil</code>. Ruby sets <code>cause</code> automatically when an exception is <strong>raised</strong> inside a <code>rescue</code> block, and our runner <em>constructs</em> <code>StepFailed.new(step, e)</code> without raising it. Constructing is not raising, so no chain was recorded.</p>
          ...
          <p>The general lesson is worth more than the fix: when your library wraps exceptions, keep the original in a field you control. Implicit chaining is a convenience for code that raises immediately, not a substitute for explicit data.</p>
        </div>
        ...
        <p>A <code>when_failed</code> handler is user code, running at the worst possible moment, and it frequently does something that can fail (send an email, post to Slack). If it raises and we do not catch it, the user sees the notifier's error instead of the actual failure, which is the single most annoying bug class in error-handling code. Catch it, log it, and preserve the original failure. The same applies to <code>always</code>.</p>
        <h3>Running it</h3>
        ...
        <p>Exponential backoff visible in the delays (0.01 then 0.02), the attempt count carried into the result, the handler receiving the original <code>RuntimeError</code> rather than our wrapper, <code>always</code> running after the failure, and the run returning rather than raising.</p>
        <div className="exercise">
          <h5>Exercise 6</h5>
          <ol>
            <li><strong>Per-step retry.</strong> Allow a step to override the pipeline policy: <code>fetch from: url, retry: {'{'} times: 5, backoff: :linear {'}'}</code>. The <code>retry:</code> key must not be passed on to the step implementation.</li>
            <li><strong>Continue on error.</strong> Add <code>continue_on_error</code> to the DSL so a failed step records its failure and the pipeline proceeds with <code>nil</code> as the payload. The <code>RunResult</code> must report <code>ok: false</code> while still containing every step's result.</li>
            <li><strong>Timeouts.</strong> Add <code>timeout: 2</code> per step. Research <code>Timeout.timeout</code> before using it, and write down in a comment why it is dangerous.</li>
          </ol>
        </div>
        <details>
          <summary>Solution 6 — open after trying</summary>
          <p><strong>1. Per-step retry.</strong> The cleanest place is the step node, not the options passed through. Strip the key when building:</p>
          ...
          <p>Then the runner builds a chain per step rather than per run, using the step's policy if present and the pipeline's otherwise. Note the cost: chains are no longer built once, so either cache them by policy or accept the allocation. Caching by policy value is easy precisely because <code>RetryPolicy</code> is a <code>Data</code> with value equality.</p>
          <p><strong>2. Continue on error.</strong> Add a handler node of kind <code>:continue_on_error</code>, and in the runner's rescue:</p>
          <pre><code>{"          if pipeline.handler(:continue_on_error)\n            context = step_context.advance(nil)\n            next\n          end\n"}</code></pre>
          <p>with the failure still recorded in <code>results</code> and <code>ok</code> computed at the end as <code>results.none? {'{'} |r| r.status == :failed {'}'}</code>. The subtlety is that later steps now receive <code>nil</code>, so this feature is only safe for pipelines whose steps tolerate it. A better design lets each step declare whether it can start from nothing, which is again plugin metadata.</p>
        ...
        <h3>Concepts</h3>
        <p>Class macros built with <code>define_method</code>, the <code>inherited</code> hook, registry metadata, generating methods per registry generation, <code>Module#prepend</code> for instrumentation, and <code>did_you_mean</code>.</p>
        <h3>Design</h3>
        <p>A block is the right way to register a two-line step. It is the wrong way to register one with five documented options, defaults, and a docstring. So plugins get a class:</p>
        ...
        <ul>
          ...
          <li><code>Plugin.call</code> constructs a fresh instance per invocation, so a step can use instance variables freely during one call without leaking between runs. Cheap, and it removes a whole class of bug.</li>
        </ul>
        ...
        <p>Milestone 4's validator reflected on a callable's <code>parameters</code>, which works for lambdas and fails for class-based steps whose <code>call</code> takes <code>**options</code>. Now a declared spec is used when present and reflection is the fallback. <strong>Declared metadata beats inferred metadata whenever it exists</strong>, and offering both means neither kind of step author is penalised.</p>
        ...
        <p><code>Class.new(ASTBuilder)</code> creates an anonymous subclass at run time; classes are objects, so this is an ordinary constructor call. Every registered verb gets a real method on it, and the class is cached until the registry changes, which is what the <code>generation</code> counter on the registry tracks.</p>
        <p>Now <code>method_missing</code> only sees names that are <em>not</em> steps, which turns it from a catch-all into a precise error detector:</p>
        ...
        <p>File, line, the mistake, and the fix, at build time, from a dynamic language. <code>DidYouMean</code> ships with Ruby (it is what produces "did you mean?" on ordinary <code>NoMethodError</code>s) and its <code>SpellChecker</code> is a public class you can point at any dictionary.</p>
        <p><code>strict: false</code> remains available and is what the data-only path and the validator tests use: build anything, check later. Two modes, one for authoring and one for tooling.</p>
        <div className="cmp">
          ...
          <p>The combination is the point. Generate what you know; keep <code>method_missing</code> for the genuinely open cases and for turning the unknown into a good error message rather than a silent acceptance.</p>
        </div>
        ...
        <p><code>prepend</code> inserts a module <em>before</em> the class in the ancestor chain, so <code>CountCalls#call</code> runs first and <code>super</code> continues to the original. Prepending to <code>singleton_class</code> targets the class method. This is how modern Ruby does monkey-patching: instead of reopening a class and aliasing the old method away (the <code>alias_method_chain</code> era), you prepend a module and call <code>super</code>, which composes cleanly with other people doing the same thing.</p>
        <div className="warn">
          <h5>A third bug I hit: the silent failed run</h5>
          <p>My first version of that module wrote <code>self.class.step_name</code>. Inside a method prepended to the <em>singleton</em> class, <code>self</code> is the class itself, so <code>self.class</code> is <code>Class</code>, which has no <code>step_name</code>, and every invocation raised <code>NoMethodError</code>.</p>
          <p>The counter printed <code>{'{'}{'}'}</code> and nothing else looked wrong, because the runner had dutifully turned each failure into a <code>RunResult</code> with <code>ok: false</code> that my demo never checked. <strong>Returning results instead of raising is the right design and it has this cost</strong>: an ignored result is an ignored error. The fix in the demo was to print <code>results.map(&:ok?)</code>, and the fix in real code is to make ignoring a result awkward, either by using <code>run!</code> at the top level or by having your CLI exit non-zero on <code>ok == false</code>.</p>
        </div>
        ...
        <p>Generated by walking <code>registry.known</code> and reading each entry's spec and doc. Twelve lines of code, and it is the beginning of <code>automation --help</code>, of a generated reference page, and of editor completion data. <strong>Metadata you declare once can be used by things you have not written yet</strong>, which is the entire argument for the plugin class over a block.</p>
        <div className="exercise">
          <h5>Exercise 7</h5>
          <ol>
            <li>
          ...
          <p>For part 2, look at <code>ObjectSpace</code> or at the <code>inherited</code> hook and decide which you prefer, then write down why.</p>
        </div>
        <details>
          ...
          <p><code>filter_map</code> (Ruby 2.7+) maps and drops <code>nil</code>s in one pass, which is exactly the shape of "collect the problems". Note that <code>value.is_a?(expected)</code> handles subclasses correctly, so <code>type: Numeric</code> accepts an Integer.</p>
          ...
          <p>Why the hook rather than <code>ObjectSpace.each_object(Class)</code>: the hook is exact, cheap and ordered, while <code>ObjectSpace</code> walks every class in the process, is slow, and is not available on all Ruby implementations (JRuby restricts it). Reaching for <code>ObjectSpace</code> is usually a sign that you missed a hook.</p>
          <p>Two details that matter more than they look. <code>rescue</code> catches <code>ScriptError</code> as well as <code>StandardError</code>, because a plugin with a syntax error raises <code>SyntaxError</code>, which is <em>not</em> a <code>StandardError</code> and would otherwise escape. And returning the failures rather than logging them lets the CLI decide whether a broken plugin is fatal.</p>
        ...
        <h2 className="milestone-head"><span className="num">Milestone 8</span>Real work: HTTP, a store, and tests that mean something</h2>
        <h3>Goal</h3>
        <p>The four shipped steps do real work. <code>fetch</code> makes an HTTP request, <code>save_to</code> writes to a persistent store, <code>summarize</code> calls a pluggable summariser, and every one of them is tested against real behaviour rather than a mock of our own code.</p>
        <h3>Concepts</h3>
        <p>Configuration as an explicit frozen object, adapters as a seam, mapping transport failures onto one error class, and testing with a real server instead of a stub.</p>
        <h3>Design</h3>
        <p><strong>Config is read once and frozen.</strong> A step that reads <code>ENV</code> itself cannot be tested twice with different settings, and in a library it is worse: your users cannot configure you without setting global state.</p>
        ...
        <p><code>PStore</code> is a standard-library key-value store with transactions, backed by <code>Marshal</code>. It costs nothing to depend on, it is genuinely transactional, and the three-method interface means replacing it with SQLite later touches one file. That is the point of an adapter: <strong>choose the boring dependency, behind a seam.</strong></p>
        <p>Its real limits, which belong in your README rather than in a surprise: the whole collection is read and written on every transaction, so it does not scale past a few megabytes, and there are no queries.</p>
        ...
        <p>Two lambdas and a documented signature, instead of an abstract base class. Anyone can pass their own, including one that calls a language model. The regex uses a lookbehind <code>(?{'<'}=[.!?])</code> to split <em>after</em> sentence-ending punctuation while keeping it, which is a technique Course 3 will use rather more aggressively.</p>
        ...
        <p><code>TCPServer.new("127.0.0.1", 0)</code> asks the kernel for any free port, and <code>@server.addr[1]</code> reports which one it chose. That is the trick that makes socket-based tests safe to run in parallel and on any machine: never hard-code a port.</p>
        <p>Why a real server rather than stubbing <code>Net::HTTP</code>: a stub tests that you called the method you think you called. A real socket tests the URL you built, the headers you sent, the redirect you followed, the timeout you set and the error you mapped. It costs about thirty lines and catches an entire category of bug that mocks are structurally unable to see.</p>
        ...
        <p><code>test_dry_run_touches_no_network</code> is my favourite test in the project. The claim "dry run does not execute anything" is easy to assert weakly (the payload is unchanged) and hard to assert convincingly. Checking that the server received <strong>zero requests</strong> is proof rather than evidence, and it would survive a refactor that moved the dry-run check somewhere wrong.</p>
        <p>The retry test asserts a time bound as well as the attempt count, because a retry policy with the wrong units (seconds instead of milliseconds) passes every correctness assertion and makes your suite take three minutes.</p>
        <div className="warn">
          <h5>The fourth bug: instance variables do not survive <code>instance_eval</code></h5>
          <p>Three of those tests failed the first time with <code>undefined method 'url' for nil:NilClass</code>. The cause:</p>
          <pre className="bad"><code>{"run_it(pipeline { fetch from: @server.url })   # @server is nil here"}</code></pre>
          <p>Inside the DSL block, <code>self</code> is the builder, so <code>@server</code> reads the <em>builder's</em> instance variable, which does not exist and is therefore <code>nil</code>. Recall the rules from Milestone 3, now complete:</p>
          <table className="grid">
            <tbody>
              <tr>
          ...
          <p>Instance variables are the nastiest of the four because they fail silently: Ruby returns <code>nil</code> for an unset one rather than raising. The fix is to bind what you need to a local before the block (<code>url = @server.url</code>), and the deeper point is that this is a permanent tax your users will pay too. Document it.</p>
        </div>
        <h3>End to end</h3>
        ...
        <div className="exercise">
          <h5>Exercise 8</h5>
          <ol>
            <li><strong>Pagination.</strong> Extend <code>fetch</code> to follow <code>Link: {'<'}...{'>'}; rel="next"</code> headers up to a configurable page limit, accumulating records. Test it with a fake server that serves two pages.</li>
            <li><strong>A second store.</strong> Write <code>Automation::JSONStore</code> with the same three methods, backed by a JSON file. Then write a <em>shared contract test</em> that runs the same assertions against both stores, and run it for each.</li>
            <li><strong>Idempotent saves.</strong> Give <code>save_to</code> a <code>key:</code> option naming a field, so re-running a pipeline updates existing records instead of appending duplicates. Prove it with a test that runs the same pipeline twice and asserts the count.</li>
          </ol>
          <p>Part 2 is the one to spend time on: a contract test is how you keep two implementations honest, and it is a technique most people meet far too late.</p>
        </div>
        <details>
          <summary>Solution 8 — open after trying</summary>
          <p><strong>2. The shared contract test</strong>, in minitest, using a module of tests included by two small classes:</p>
          ...
          <p>A module of test methods, included into several test classes, each of which supplies the setup: that is the whole technique, and it is why Ruby's mixins matter for testing as much as for production code. (RSpec spells it <code>shared_examples</code>, which is the same idea with more syntax.)</p>
        ...
        <div className="warn">
          <ul>
            ...
            <li><strong>Forgetting that the store swallows a failure</strong> when the run result is never checked. </li>
          </ul>
        </div>
        <h4>Checkpoint</h4>
        <ol>
          ...
          <li>Why is "the fake server received zero requests" a better dry-run assertion than "the payload is unchanged"?</li>
        ...
        <div className="why">
          <h5>Why are we using this language here?</h5>
          <p>Milestone 7 is the clearest case yet: <code>option :max_words, default: 10</code> declares metadata and generates a reader in one line, and the same declaration then drives validation, documentation and editor-facing introspection. In a static language you would either write the metadata twice (annotations plus fields) or generate code from a schema. The class-macro pattern is Ruby at its best, and once you can write one you can read Rails.</p>
          <p>Milestone 8 is where Ruby is merely adequate. The HTTP client is fine, <code>PStore</code> is a reasonable default with real limits, and none of this is better than what Python, Go or Java would give you. The gem's dependency-free standard library is genuinely pleasant; the performance is not a consideration here only because every step is I/O-bound.</p>
          <p>And the bug tally for these four milestones is the honest scoreboard: a silently shadowed verb, an exception chain that was never recorded, a prepended method whose <code>self</code> was not what I assumed, and three tests broken by instance variables inside <code>instance_eval</code>. Every one of them is a dynamic-language failure that a compiler would have caught or made impossible. The DSL is worth it; the tests are what make it safe to be worth it.</p>
        </div>
