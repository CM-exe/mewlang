    ...
    <div className="theme-ruby">
      <div className="wrap">
        <header className="masthead">
          ...
          <p className="lede">The gem ships testing helpers for its own users, learns to explain and diagram itself, gains the ability to rewrite pipelines into new ones, and becomes a command you can install.</p>
        </header>
        <div className="note">
          <h5>Verification note</h5>
          <p>Ruby 3.2.3. The suite finishes at 34 tests and 94 assertions, all passing, and every CLI transcript below is real output from <code>exe/automation</code>. Two new bugs are documented, one of which is the Milestone 6 shadowing trap catching me a second time in my own test file.</p>
        </div>
        ...
        <table className="grid">
          <tbody>
            ...
            <tr>
              <td>Assertions</td>
              <td><code>assert_steps</code>, <code>assert_run_failed</code> and friends, so failures read usefully </td>
            </tr>
          </tbody>
        </table>
        <p>Note the choice of <strong>spy over mock</strong>. A mock asserts expectations up front ("you will be called once with these arguments") and fails inside the code under test, producing confusing backtraces. A spy records what happened and lets you assert afterwards, in the test, where the failure message belongs. For a pipeline, where the interesting question is usually "what did the third step actually receive", the spy wins easily.</p>
        ...
        <p>Twenty lines and no dependency on a mocking library. It works because the registry accepts <em>anything with <code>#call</code></em>, a decision made back in Milestone 1 that keeps paying.</p>
        ...
        <p><code>@test_registry ||= Registry.new</code> gives every test its own registry, created on first use and discarded with the test instance. <strong>No global state means no teardown and no order dependence</strong>, which is worth more than any amount of careful cleanup code.</p>
        ...
        <div className="warn">
          ...
          <p>I documented this exact hazard in Milestone 6 and then walked into it again ten minutes later, which tells you how easy it is. Two conclusions. First, the naming convention matters: name the spy <code>upcaser</code>, <code>fetch_spy</code>, anything that is not the verb. Second, and more usefully, <strong>this is a design smell in the DSL, not only in the test</strong>. A language where an identifier can silently mean either a verb or a variable will bite your users. If I were shipping this seriously, <code>build_pipeline</code> would compare the resulting step list against the verbs mentioned in the block source, or at minimum the documentation would carry a warning with this example in it.</p>
        </div>
        <div className="warn">
          <h5>A duck-typing bug the helpers exposed</h5>
          <p>Adding <code>Recorder</code> broke four tests with <code>undefined method 'parameters' for #{'<'}Recorder{'>'}</code>. The validator from Milestone 4 assumed every callable answers <code>parameters</code>, which is true of Procs, lambdas and Methods, and false of an arbitrary object that merely has <code>#call</code>.</p>
          ...
          <p>The lesson is general: <strong>if you accept a duck, check every feather you use.</strong> We advertised "anything responding to <code>#call</code>" and then quietly required a second method. <code>impl.method(:call).parameters</code> is the correct general answer, and returning <code>nil</code> to mean "cannot check" is better than raising, because a validator that crashes on an unusual step is worse than one that declines to judge it.</p>
          <p>While investigating I also found a latent hazard in Milestone 7's builder-class cache, which was keyed on <code>registry.object_id</code>. Object ids are <em>recycled</em> after garbage collection, so two different registries can share one and the cache can hand out the wrong verbs. The cache now lives on the registry itself. <strong><code>object_id</code> is not a durable identity</strong>, in Ruby or anywhere else with a moving or reusing allocator.</p>
        </div>
        <div className="cmp">
          ...
          <p>The helpers work in both because they are plain methods in a module, which is the right way to ship test support: no dependency on a framework, no <code>RSpec.configure</code> in your gem.</p>
          <p>One RSpec-specific hazard worth knowing: <code>let</code> is lazy, so a spy defined in a <code>let</code> is not registered until something references it, and a pipeline built before that reference will not see the step. Minitest's eager <code>setup</code> has the opposite trade-off. Neither is wrong; both surprise people once.</p>
        </div>
        <div className="exercise">
          <h5>Exercise 9</h5>
          <p>Ship a <strong>plugin contract test</strong>: a module a third-party plugin author includes to check their plugin behaves like a good citizen. It should verify that the plugin declares a <code>step_name</code>, that every required option is genuinely required (constructing without it and calling raises or the validator complains), that <code>call</code> with an empty input does not raise, that building a pipeline containing it executes nothing, and that it does not mutate its input.</p>
          <p>Requirements: the author writes only <code>def plugin_class = MyPlugin</code> and <code>def valid_options = {'{'}from: "x"{'}'}</code>. Include a test that your own <code>Steps::Filter</code> passes it, and one that a deliberately bad plugin fails it.</p>
        </div>
        <details>
          ...
          <ul>
            <li><strong>A contract test is a specification you can execute.</strong> Everything a plugin must do is written once, checked automatically, and available to people who have never read your source. This is how a plugin ecosystem stays coherent without a maintainer policing it.</li>
            <li><strong>The immutability check is the valuable one.</strong> A step that mutates the records it is given breaks the next author's pipeline in a way that is very hard to trace, and no amount of documentation prevents it. Freezing the input in the test and comparing a deep copy catches it mechanically.</li>
            <li><strong><code>public_send(plugin_class.step_name, ...)</code></strong> is how a test calls a verb whose name it does not know until run time. Inside the DSL block that is exactly what the generated methods make possible.</li>
        ...
        <p>Generating Mermaid rather than an image means the output is text: diffable, greppable, renderable by GitHub without a build step, and editable by someone who does not have your gem installed. When you need a diagram from a program, emitting a text format that something else renders is almost always better than emitting pixels.</p>
        ...
        <p>This works because <code>StepNode</code> is a <code>Data</code>, so <code>old.options == new.options</code> compares contents. Had the nodes been ordinary objects without value equality, every comparison would be identity and every step would look changed. <strong>The diff is three lines of logic and one line of data modelling done four milestones earlier.</strong></p>
        <p>Note what this deliberately is not: a real diff algorithm. It matches steps by name, so a pipeline with two <code>fetch</code> steps will confuse it, and it reports reordering as a boolean rather than as moves. For a change-review tool that is fine; if you needed better, the right move is a proper sequence alignment (the same algorithm <code>git diff</code> uses), not more special cases.</p>
        <div className="exercise">
          <h5>Exercise 10</h5>
          <ol>
            <li><strong>Handle duplicates.</strong> Make <code>diff</code> correct for pipelines containing two steps with the same name. Decide whether to match by position, by options, or by a stable identity you add to <code>StepNode</code>, and write down why.</li>
            <li><strong>A review command.</strong> Add <code>automation diff FILE PIPELINE --against REF</code> that loads the pipeline from a git revision (<code>git show REF:FILE</code>) and prints the diff. Exit non-zero when anything changed, so it can be a CI check.</li>
            <li><strong>Cost annotations.</strong> Let a plugin declare <code>cost :network</code> or <code>cost :expensive</code>, and have <code>explain</code> summarise how many network calls a pipeline will make before it runs.</li>
          </ol>
        </div>
        <details>
          ...
          <p>Matching by position breaks the moment someone inserts a step at the top; matching by options means a step whose options changed looks like an add plus a remove, which is exactly what a diff should avoid saying. An identity derived from the source location is stable across edits elsewhere in the file and is already available. The cost is that a rewritten step needs a new id, and you must decide whether <code>set_options</code> preserves it (it should) while <code>replace</code> does not.</p>
          <p>This is the same problem React solves with <code>key</code> props and that database migrations solve with explicit identifiers, and the answer is always the same: <strong>if you want to diff a sequence, put identity in the elements rather than inferring it.</strong></p>
          <p><strong>2.</strong> The interesting part is loading two versions of the same pipeline into one process. Since <code>Automation.define</code> registers by name, loading the old version overwrites the new one. Fix it by loading each into its own registry and pipeline table, or more simply by shelling out to a subprocess that prints <code>to_h</code> as JSON and comparing the data:</p>
        ...
        <h3>Goal</h3>
        <p>A rewrite API that produces new pipelines from old ones, organisation-wide policies applied to every pipeline as it is defined, and a pipeline that improves its own successor based on measurements of its last run.</p>
        <h3>Concepts</h3>
        <p>Transformation as a pure function, a second small DSL layered on the first, safety limits on generated structure, and the difference between modifying a running program and producing its replacement.</p>
        <h3>Design</h3>
        <p>"Dynamically modifiable pipelines" can mean two very different things, and the distinction matters more than the implementation.</p>
        <table className="grid">
          <tbody>
            <tr>
              <th></th>
        ...
        <p>We take the second, and it is not a compromise. <strong>Everything people actually want from self-modifying pipelines (adaptive optimisation, feature flags, A/B variants, policy enforcement) is achievable by generating a new pipeline value</strong>, and the one thing it rules out (a step rewriting the steps after it while they are in flight) is a debugging nightmare nobody should want.</p>
        <h3>Implementation</h3>
        ...
        <p>Each operation is a lambda from a step list to a step list, collected rather than applied immediately, and then folded in <code>apply</code>. That buys two things: nothing happens until <code>apply</code>, so a rewrite can be inspected or abandoned; and the fold means a failed operation halfway through leaves the original untouched, because we were never mutating it.</p>
        <p><code>MAX_STEPS</code> is the boring safety feature that matters. A policy with a bug can append a step every time it runs, and without a limit the first symptom is memory exhaustion. Any code that generates structure needs a bound on the structure it generates.</p>
        ...
        <p><code>class_eval</code> reopens the generated <code>Data</code> class to add a method, which is the class-reopening that Ruby is famous for, used here on a class we own. And <code>instance_eval</code> appears a second time, making a second DSL: the vocabulary inside a <code>rewrite</code> block is <code>insert_before</code>, <code>remove</code>, <code>step</code>. Once you have the technique, layering little languages becomes routine, which is a large part of why Ruby codebases look the way they do.</p>
        ...
        <p>A user wrote <code>max_words: 5000</code> and got 50, and never asked for a logging step but has one. This is genuinely powerful and genuinely dangerous, so two observations.</p>
        <p>First, <strong>the idempotence check in <code>always_log_first</code> is not optional</strong>. Without <code>pl.step_names.first == :log_start ?</code>, every re-application prepends another step, and since policies run on every <code>define</code>, a pipeline rewritten and re-registered grows without bound. <code>MAX_STEPS</code> would eventually stop it, which is a crash rather than an answer. A policy should be a function whose second application changes nothing.</p>
        <p>Second, <strong>a policy that silently changes what a user wrote is a debugging trap.</strong> <code>explain</code> mitigates it (the injected step shows a location of <code>(rewritten)</code>), and a serious version would record which policy made each change and print that. Governance you cannot see is governance people fight.</p>
        ...
        <div className="exercise">
          <h5>Exercise 11</h5>
          <ol>
            <li><strong>Fixpoint and idempotence.</strong> Apply policies repeatedly until the pipeline stops changing, with a limit of, say, 10 rounds. If the limit is hit, raise an error naming the policies that are still changing things. Then add a test that a deliberately non-idempotent policy is caught rather than looping.</li>
            <li><strong>Provenance.</strong> Record which policy or rewrite introduced each step, and show it in <code>explain</code> as <code>(added by :always_log_first)</code>.</li>
            <li><strong>Opt out.</strong> Let a pipeline declare <code>skip_policy :cap_summaries</code>, and decide (and document) whether policies should be skippable at all.</li>
          </ol>
        </div>
        <details>
          ...
          <p><strong><code>after == pipeline</code> is the whole trick</strong>, and it only works because the AST is made of <code>Data</code> objects with value equality. A fixpoint loop over mutable objects would need a hand-written comparison, and would probably get it wrong.</p>
          <p>Naming the culprits in the error is what turns a frustrating message into a fixable one: applying each policy once more and reporting which ones still change something identifies the offender exactly.</p>
        ...
        <div className="warn">
          <ul>
            ...
            <li><strong>Mutating the step array</strong> inside a rewrite operation instead of returning a new one. <code>FrozenError</code> if you froze, silent corruption if you did not.</li>
            ...
            <li><strong>Invisible transformations.</strong> If <code>explain</code> cannot show that a policy changed something, users will assume the tool is broken.</li>
        ...
        <ul>
          ...
          <li><strong><code>rubygems_mfa_required</code></strong> makes it impossible to publish a version of your gem without two-factor authentication. Supply-chain attacks on package registries are routine; this is one line.</li>
        ...
        <ul>
          <li><strong>Every method returns an exit code</strong> and <code>exit</code> happens exactly once, at the bottom of the file. That makes the whole CLI testable by calling <code>CLI.new.call(%w[run file.rb])</code> and asserting on the integer.</li>
          <li><strong>Errors go to <code>$stderr</code> via <code>warn</code>, results to <code>$stdout</code>.</strong> Then <code>automation explain x.rb | less</code> works and error messages are not swallowed by a pipe.</li>
          <li><strong>Exit codes are an interface:</strong> 0 success, 1 the pipeline failed, 2 you invoked me wrongly. A CI job depends on this distinction.</li>
          <li><strong>A dispatch Hash beats a <code>case</code></strong> here because it doubles as the list of valid commands for the usage message.</li>
        ...
        <p>The failure is the point: the JSON pipeline really ran, through the real steps, and failed at the network rather than at the parser.</p>
        ...
        <p>Three attempts (the <code>retry_on</code> in the file), the <code>when_failed</code> handler's warning on stderr, the result on stdout, exit code 1.</p>
        <h3>Releasing</h3>
        ...
        <p><code>rake release</code> comes from bundler's gem tasks and does the whole sequence, refusing if the working tree is dirty. Two conventions worth following: <strong>the version constant is the single source of truth</strong> (the gemspec reads it, so they cannot disagree), and <strong>the CHANGELOG entry is written before the release, not after</strong>, because "what changed" is much easier to answer while you remember.</p>
        <p>Semantic versioning for a DSL gem deserves a thought. Your public API is not only your Ruby methods: it is <em>the verbs, their options and their behaviour</em>. Renaming a step option is a breaking change even though no Ruby method signature changed. Being explicit about that in your README saves an argument later. </p>
        <div className="exercise">
          <h5>Exercise 12</h5>
          <ol>
            <li><strong>Test the CLI.</strong> Write an integration test that calls <code>Automation::CLI.new.call(...)</code> with captured stdout and stderr, asserting output and exit codes for: a successful dry run, a validation failure (exit 1), and a bad invocation (exit 2). Use <code>capture_io</code>, which minitest provides.</li>
            <li><strong>Machine-readable output.</strong> Add <code>--format json</code> so <code>run</code> and <code>explain</code> emit JSON for CI consumption. Decide what the schema is and version it.</li>
            <li><strong>A generator.</strong> Add <code>automation init</code> writing a starter pipeline file and a plugin skeleton, then verify that the generated files pass <code>automation explain</code> and the plugin contract test from Exercise 9.</li>
          </ol>
        </div>
        <details>
          ...
          <p>Three things this makes possible that testing by shelling out does not: it runs in-process so coverage tools see it, it is fast enough to run on every save, and it fails with a real backtrace instead of "expected exit 0, got 1".</p>
        ...
        <div className="why">
          ...
          <p>Milestone 11 is the last strong Ruby argument in this course. Two DSLs in one system (one for describing pipelines, one for rewriting them), both built from <code>instance_eval</code> and both under thirty lines, with value objects giving fixpoint detection for free. Writing that in a static language means a builder API, an explicit visitor, and generated equality; Ruby lets it stay small enough that a reader can hold it all at once.</p>
          <p>Milestones 9, 10 and 12 are Ruby being solid rather than special. The testing helpers rely on duck typing being real, which is nice; the inspector is plain data transformation; the CLI and gemspec are conventional and pleasant. A Python or TypeScript version of these three would look similar and be about as good.</p>
          <p>Course-wide honesty: the total bug count I hit while writing these twelve milestones is six, and every one of them was a dynamic-language failure. A shadowed local eating a verb (twice), an exception chain that was never recorded, a prepended method whose <code>self</code> I mistook, instance variables vanishing inside <code>instance_eval</code>, a duck that lacked a feather I used, and a method silently redefined by a second definition of the same name. None would have compiled in Go. All were caught by tests, in seconds, and the fixes were small. <strong>That trade, more errors caught later but caught cheaply, with far more expressive power in exchange, is what choosing Ruby actually means.</strong> </p>
        </div>
