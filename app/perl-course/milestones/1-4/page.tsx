  ...
  return (
    <div className="theme-perl">
      <div className="wrap">
        ...
        <h4>Experiment</h4>
        <p>Add <code>print STDERR "$ARGV:$.\n";</code> right after <code>$lines++;</code> and run <code>strata-scan</code> over two small files at once. <code>$.</code> does not reset at the file boundary: the second file's first line reports as continuing the count from the first, not restarting at 1. Add <code>close ARGV if eof;</code> at the bottom of the loop and run again, and each file now starts back at line 1. Milestone 2's <code>push @first_failures, "$ARGV:$." if ...</code> depends on exactly this number meaning "line within this file", so which behaviour you want is a decision to make deliberately, not a default to inherit by accident.</p>
        ...
        <div className="cmp">
          <h5>Four passes over the data vs one</h5>
          <pre className="plain"><code>{"pandas                                      Perl\n──────                                      ────\ndf = pd.read_csv(log, sep=None)             while (my $line = <>) {\ntop = df.groupby(\"ip\").size() \\                 $hits{$f{ip}}++;\n        .sort_values(ascending=False)           $errors{$f{ip}}++ if $f{status} >= 400;\nerrs = df[df.status >= 400] \\                   $by_class{...}++;\n        .groupby(\"ip\").size()                   $by_minute{...}++;\nclasses = df.status.astype(str).str[0]      }\nby_min = df.ts.dt.floor(\"min\") \\\n        .value_counts()"}</code></pre>
          <p>The pandas version is not wrong, and for a file that fits comfortably in memory it reads well: one line per question. But it makes four full scans of the loaded table (five, counting the parse), and every one of them assumes the whole thing is already sitting in RAM. The Perl loop makes one pass over the stream and updates four hashes as it goes, holding nothing but the current line and a handful of counters at any moment. On a file that does not fit in memory — which is the normal case for a real log — only one of these approaches runs at all.</p>
        </div>
        <div className="exercise">
          <h5>Exercise 2</h5>
          <ol>
        ...
        <div className="cmp">
          <h5>Splitting on whitespace vs a pattern that knows about quotes</h5>
          <pre className="plain"><code>{"awk (typical)                                     Perl\n─────────────                                     ────\nawk '{print $1, $7, $9}' access.log               if ($line =~ $ACCESS) {\n  # breaks the moment a quoted field                   my %f = %+;\n  # contains a space, e.g. a request                   # $f{request} is the whole\n  # line with a spaced-out query string                # quoted field, spaces and all\n"}</code></pre>
          <p><code>awk</code>'s default field splitting has no concept of quoting: a request line like <code>"GET /search?q=hello world HTTP/1.1"</code> — a real, if unusual, query string — splits into extra fields and shifts every column after it, silently corrupting the status code and byte count columns awk thinks it is reading. The regex's <code>[^"]*</code> group captures the whole quoted field as one named capture whatever it contains, because it is driven by the quote characters rather than by whitespace. The awk one-liner is shorter to type and just as fast on a well-formed line; it is answering a simpler question than the log format actually asks, and the gap only shows up on the inputs you did not think to test by hand.</p>
        </div>
        <div className="exercise">
          <h5>
        ...
        <h4>Experiment</h4>
        <p>Change <code>(?{'<'}bytes{'>'}\d+|-)?</code> in <code>$ACCESS</code> to drop the trailing <code>?</code>, making the byte count mandatory, and re-parse the line from the Milestone 2 bug report: <code>... "GET /api/export HTTP/1.1" 200</code> with no trailing bytes field. It no longer matches at all: the record Milestone 3 exists to rescue — status, method and path preserved, only <code>missing_bytes</code> flagged — goes back to being indistinguishable from <code>no_match</code> garbage. That single <code>?</code> is the entire difference between "lossy but usable" and "silently gone", worth watching fail once before trusting it in the four parsers still to come.</p>
        ...
        <div className="cmp">
          <h5>Piping processes vs piping closures</h5>
          <pre className="plain"><code>{"shell pipeline                                Perl Pipeline\n───────────────                                ─────────────\ngrep ERROR access.log \\                        $pipe->add_stage($match_stage)\n  | awk '{print $1}' \\                              ->add_stage($counter_stage)\n  | sort | uniq -c \\                                ->add_stage(drop_problems());\n  | sort -rn\n                                                $pipe->run_handle($fh, $name);"}</code></pre>
          <p>The shell version needs no Perl at all and is genuinely the right tool for a quick count. But each stage is a separate process re-parsing plain text with no memory of where a field came from — by the time <code>uniq -c</code> runs, the line number and source file of the original match are gone, and there is no way to ask "which raw line produced this row" afterwards. The Perl <code>Pipeline</code> carries every record as an object with its provenance intact through every stage, in one process, at the cost — as the throughput numbers later in this milestone show plainly — of being measurably slower than either the shell pipeline or a flat script. Composability and forensic traceability are what that slowdown buys, not abstraction for its own sake.</p>
        </div>
        <div className="exercise">
          <h5>
        ...
        <h4>Experiment</h4>
        <p>Take any stage and replace its final <code>return $rec;</code> with a bare trailing <code>print STDERR "stage ran\n";</code> — exactly the mistake the common-mistakes box below warns about — and put it first in a chain that has a second stage after it. The second stage does not receive the record: it receives the return value of <code>print</code>, which is <code>1</code>, and the moment it calls <code>$rec-{'>'}field(...)</code> on that plain scalar, Perl dies with <code>Can't locate object method "field" via package "1"</code>. A stage that happened to be <em>last</em> in the chain would have hidden this entirely, since the pipeline only checks truthiness for counting records versus drops — which is exactly why this mistake is easy to introduce and easy not to notice until stages get reordered.</p>
