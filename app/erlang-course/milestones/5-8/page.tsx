    ...
    <div className="theme-erlang">
      <div className="wrap">
        ...
        <div className="why">
          ...
          <p><code>gen_server</code> is a contract, not magic: everything it does, Milestone 3's hand-rolled <code>loop/1</code> already did, by hand, in fifteen lines. What OTP buys is that the fifteen lines are written once, in the standard library, tested across decades of production systems, and a bug in them is nobody's bug in this project ever again. The honest cost is a real one: a behaviour adds a layer of indirection between "what message arrived" and "what code runs" that a newcomer has to learn before <code>mesh_node.erl</code> makes sense at a glance — Milestone 3's raw <code>receive</code> loop, for all its manual plumbing, is more immediately legible to someone who has never seen <code>gen_server</code> before. That tradeoff — less code and fewer footguns, in exchange for needing to already know the convention — is the same one every framework built on "implement these callbacks, we call the shots" makes, in any language.</p>
        </div>
        ...
        <h4>Experiment</h4>
        <p>Call <code>gen_server:call/3</code> directly, bypassing <code>get_status/1</code>'s wrapper, with a timeout of <code>0</code> milliseconds. Predict whether it behaves like Milestone 3's hand-rolled <code>{'{'}error, timeout{'}'}</code> return value.</p>
        <pre className="plain"><code>{"1> {ok, Pid} = mesh_node:start_link(1).\n{ok,<0.83.0>}\n2> catch gen_server:call(Pid, get_status, 0).\n{'EXIT',{timeout,{gen_server,call,[<0.83.0>,get_status,0]}}}\n"}</code></pre>
        <p>It does not. Milestone 3's hand-rolled <code>call/2</code> returned <code>{'{'}error, timeout{'}'}</code> as an ordinary value the caller could pattern-match on and keep going. <code>gen_server:call/2,3</code> instead <strong>exits</strong> the calling process on timeout — an ordinary value from Milestone 3 becomes a crash here unless the caller explicitly wraps the call in <code>catch</code> or <code>try</code>, which is a real, easy-to-miss behavioural difference between the two versions despite the cmp box above describing them as "almost the identical syntax."</p>
        <h4>Common mistakes in Milestone 5</h4>
        <div className="warn">
          <p><strong>Returning a bare tuple from <code>handle_call/3</code> instead of the required <code>{'{'}reply, Reply, NewState{'}'}</code> shape.</strong> <code>gen_server</code> does not interpret <code>{'{'}Status, State{'}'}</code> as "reply with <code>Status</code>" — it is not a valid return value at all, and the callback process crashes with <code>bad_return_value</code> the moment it happens, taking down whichever caller was waiting on the reply along with it:</p>
          <pre className="plain"><code>{"** exception exit: {bad_return_value,\n                        {alive,#{id => 1,status => alive,energy => 100}}}\n      in function  gen_server:handle_common_reply/8\n"}</code></pre>
          <p>The <code>reply</code> atom is not decoration — it is how <code>gen_server</code> distinguishes "here is the reply" from the other valid shapes a callback can return (<code>{'{'}noreply, State{'}'}</code>, <code>{'{'}stop, Reason, Reply, State{'}'}</code>), and leaving it off is not a smaller version of a correct answer, it is simply not one of the shapes <code>gen_server</code> recognises.</p>
        </div>
        <h4>Checkpoint</h4>
        <ol>
        ...
        <h4>Experiment</h4>
        <p>Repeat the verified transcript above — four crashes of the same node — but this time sleep 6 seconds between each crash instead of sending them back to back. With <code>period ={'>'} 5</code> unchanged, predict whether the supervisor still goes down on the fourth crash.</p>
        ...
        <p>It stays up. <code>period</code> is a genuinely <em>rolling</em> five-second window, not a running total of "crashes so far" — a restart six seconds after the previous one has already aged out of the window the previous restart was counted in, so each of these four crashes is evaluated against a window containing, at most, itself. The verified transcript earlier in this milestone and this Experiment use the exact same crash count and the exact same <code>intensity</code>/<code>period</code> numbers; only the timing between crashes differs, and that alone is the difference between "supervisor gives up" and "supervisor absorbs every one of them indefinitely."</p>
        <h4>Common mistakes in Milestone 6</h4>
        <div className="warn">
          <p><strong>Calling <code>supervisor:start_child/2</code> directly with the wrong extra-arguments list</strong>, bypassing <code>node_sup:start_node/1</code>'s own wrapper. A <code>simple_one_for_one</code> child spec's <code>start</code> MFA gets its extra arguments list appended at call time — <code>node_sup:start_node(Id)</code> always supplies <code>[Id]</code>, but calling the underlying <code>supervisor:start_child/2</code> straight from the shell with the wrong shape does not crash the supervisor, it fails the one child start cleanly: </p>
          <pre className="plain"><code>{"1> supervisor:start_child(node_sup, []).\n{error,{'EXIT',{undef,[{mesh_node,start_link,[],[]}, ...]}}}\n"}</code></pre>
          <p>Worth noticing precisely because it is the opposite lesson from most of this milestone: a supervisor treats a child that <em>fails to start</em> as an ordinary, reportable error, not as a crash to restart from — restart intensity and restart strategy only ever apply to a child that started successfully and later died, not to one that never managed to start at all.</p>
        </div>
        <h4>Checkpoint</h4>
        <ol>
        ...
        <h4>Experiment</h4>
        <p>Change <code>route/2</code>'s drop threshold from <code>R ={'<'} 0.02</code> to <code>R ={'<'} 0.5</code>, then route 1,000 messages to a single registered node and count how many come back <code>dropped</code>. Predict the rough count before running it.</p>
        <pre className="plain"><code>{"1> {ok, Pid} = mesh_node:start_link(1), mesh_registry:register_node(1, Pid).\nok\n2> Results = [mesh_registry:route(1, ping) || _ <- lists:seq(1, 1000)].\n[...]\n3> length([x || dropped <- Results]).\n512\n"}</code></pre>
        <p>At the original 2% threshold, the same measurement returns almost exactly 20 dropped out of 1,000 — the literal constant in <code>route/2</code> is not a description of behaviour, it <em>is</em> the behaviour, and changing one number in one clause head changes the mesh's effective reliability end to end, with nothing else in the registry, the nodes, or the chaos module aware anything changed.</p>
        <h4>Common mistakes in Milestone 7</h4>
        <div className="warn">
          <p><strong>Calling <code>mesh_registry:start/0</code> more than once.</strong> <code>ets:new/2</code> with <code>named_table</code> can only create a given name once per node — a second call, from re-running setup in a fresh shell session pasted over an existing one, or from a supervisor restarting whatever code happens to call <code>start/0</code>, crashes immediately:</p>
          <pre className="plain"><code>{"1> mesh_registry:start().\nok\n2> mesh_registry:start().\n** exception error: bad argument\n     in function  ets:new/2\n        called as ets:new(mesh_registry,[set,public,named_table])\n"}</code></pre>
          <p>A registry meant to be started exactly once benefits from a guard against exactly this — checking <code>ets:whereis(?MODULE)</code> first, or simply making sure <code>start/0</code> is only ever called from one place (the application's own startup, in a real supervision tree) rather than from ad-hoc shell sessions that might already have one running.</p>
        </div>
        <h4>Checkpoint</h4>
        <ol>
        ...
        <h4>Experiment</h4>
        <p>Put a node to sleep with <code>{'{'}suspend_for, 2000{'}'}</code>, the same attack <code>mesh_chaos</code> uses, then compare a fire-and-forget send against that node with a real <code>gen_server:call</code> against it, timed. Predict which one blocks.</p>
        <pre className="plain"><code>{"1> {ok, Pid} = mesh_node:start_link(1).\n{ok,<0.90.0>}\n2> gen_server:cast(Pid, {suspend_for, 2000}).\nok\n3> timer:tc(fun() -> Pid ! ping end).\n{17,ping}\n4> timer:tc(fun() -> mesh_node:get_status(Pid) end).\n{1989123,alive}\n"}</code></pre>
        <p>The bare send returns in 17 microseconds — sending a message never waits for anything on the receiving end, suspended or not. <code>get_status/1</code>, built on <code>gen_server:call</code>, takes just under two full seconds: it genuinely queues behind the sleeping <code>handle_cast</code> callback and only gets its answer once that callback returns. This is the warn box above made measurable rather than just asserted: a slow handler degrades exactly the calls that need a reply from that one process, and does nothing whatsoever to a fire-and-forget send aimed at the same pid.</p>
        <h4>Common mistakes in Milestone 8</h4>
        <div className="warn">
          <p><strong>Calling <code>mesh_chaos:run/2</code> with an <code>Opts</code> map missing one of the required keys.</strong> <code>run/2</code> pattern-matches <code>#{'{'}crash_rate := CrashRate, slow_rate := SlowRate{'}'} = Opts</code> — both keys are required, with no default for either, so a caller who only cares about crashes and reasonably assumes <code>slow_rate</code> is optional gets an immediate, unhelpful-looking crash instead of a chaos run that just never goes slow:</p>
          <pre className="plain"><code>{"1> mesh_chaos:run(1, #{crash_rate => 0.1}).\n** exception error: no match of right hand side value #{crash_rate => 0.1}\n"}</code></pre>
          <p>A map pattern with <code>:=</code> for every key it destructures is deliberately strict — it is the same "fail loudly on the genuinely unexpected shape" instinct the rest of this course applies to messages, applied here to an options map instead. The fix is supplying both keys, not loosening the pattern to guess at a missing one.</p>
        </div>
