...
export const metadata: Metadata = {
  title: "Erlang: The Internet That Never Dies — Instalment",
  description: "Course 4 of 5: build a simulated network of thousands of supervised Erlang processes that crash, restart, partition and recover, ending in a live dashboard and a real distributed cluster.",
};
    ...
    <div className="theme-erlang">
      <div className="wrap">
        <header className="masthead">
          <p className="kicker">Instalment 16 · Course 4 (Erlang) · Parts 0–2</p>
          <h1>The language that assumes everything will crash</h1>
          <p className="lede">
        ...
        <h2><span className="num">Course 4 · Part 0</span>What are we building?</h2>
        <h3>The final result</h3>
        <p>A simulated network, <code>mesh</code>, of up to several thousand independent nodes, each an Erlang process. Nodes exchange messages, maintain a small amount of state, and periodically misbehave: a node can crash outright, hang and stop responding, send another node a message it cannot parse, or simply vanish. A supervision tree watches every node and restarts the ones that die, with a policy that says precisely how much dying is tolerable before the supervisor gives up and takes the whole subtree down with it.</p>
        <p>On top of that: a registry so nodes can find each other by name instead of by process identifier: an observability layer with counters for messages sent, nodes alive, nodes restarted, and message loss: a live dashboard, served over plain HTTP from a process running inside the same system it is reporting on, showing all of that updating in real time in a browser. Then, in the back half of the course, the single-machine simulation becomes a genuine distributed system: several real operating-system processes, each running its own instance of the Erlang virtual machine (the BEAM), connected over a real network, able to survive one of those machines being killed outright.</p>
        <pre className="plain"><code>{"$ ./bin/mesh start --nodes 2000 --dashboard :8080\nmesh: 2000 nodes started, supervisor tree depth 3\nmesh: dashboard listening on http://localhost:8080\n\n$ ./bin/mesh chaos --crash-rate 0.02 --partition-every 30s\nchaos: injecting failures into a live system\n\n  alive: 1,978   restarting: 14   partitioned: 8   msgs/sec: 40,221\n"}</code></pre>
        <p>Everything in that output is a real, measured number from a real run once the project exists. Nothing here is a diagram of an idea; every milestone below produces working, running, compiled code.</p>
        <h3>Why this project is interesting</h3>
        <p>Every other course in this curriculum treats a crash as a bug to eliminate. This one treats it as a certainty to plan for. That is not a small difference in emphasis — it is a different starting axiom for the whole design. A Go service that panics takes the whole process down with it, and the discipline is to never let a goroutine panic in the first place. An Erlang process that crashes is <em>routine</em>: by the end of Milestone 4 you will have a system where processes die constantly, on purpose, in the test suite, and the assertion is never "no process died" — it is "the system as a whole kept working while processes died."</p>
        <p>That reframing is the entire subject of this course. Once you accept that any single process can and will die, the interesting engineering question stops being "how do I prevent this" and becomes "who notices, what do they do about it, and how fast." The answer Erlang and its OTP (Open Telecom Platform) library give is <strong>supervision trees</strong>: a static, declarative description of which processes depend on which, and exactly what should happen — restart just this one, restart its siblings too, give up entirely — when one of them dies. You will hand-write the naive version in Milestone 4, before you have been told what a supervisor is, discover exactly what it gets wrong, and then meet OTP's <code>supervisor</code> behaviour in Milestone 6 as the answer to a problem you have already felt.</p>
        <h3>Why Erlang in particular</h3>
        <p>Three properties of the language are not available, in combination, anywhere else in this curriculum.</p>
        <p><strong>Processes are cheap and share nothing.</strong> An Erlang process is not an OS thread and not a goroutine — it is a lightweight, independently garbage-collected unit with its own heap, started in microseconds, and a BEAM instance can comfortably run millions of them. Because no process can reach into another's memory — the only way in is a message, copied, into a mailbox — a crashed process cannot corrupt anything belonging to a process that did not crash. Go's goroutines are cheap too, but they share an address space, so a wild pointer write in one can in principle corrupt data another is reading; the <code>go</code> runtime's response to an unrecovered panic in any goroutine is to kill the entire program. Erlang's response to an unhandled exception in a process is to kill exactly that process, and nothing else, ever.</p>
        <p><strong>Let it crash is a real engineering strategy, not a slogan.</strong> Given the isolation above, the language actively discourages the defensive-programming instinct of catching every possible error and trying to handle it in place. If a node receives a message it cannot make sense of, the idiomatic move is to let the process crash — loudly, with a stack trace, to a log — and let the supervisor restart it into a known-good state, rather than trying to enumerate every way input can be wrong and patch around each one. Milestone 8 makes this concrete: you will feed the mesh genuinely malformed messages and watch the system absorb them by discarding and restarting, rather than by defending.</p>
        <p><strong>Distribution was never an afterthought.</strong> The message-passing model between two processes on one machine and two processes on different machines is, by design, close to the same API — Erlang was built at Ericsson in the late 1980s specifically to run telephone switches that were not allowed to go down, and those switches were distributed hardware from day one. Milestone 11 turns your single-machine mesh into several real operating-system processes talking over a real network with a change measured in tens of lines, not a rewrite — because the message-passing discipline you have been using since Milestone 3 was already, quietly, the distributed-systems discipline.</p>
        <div className="why">
          <h5>Why are we using this language here?</h5>
          <p>Be honest about the trade this makes. Erlang's processes are wonderful for isolation and terrible for sharing a large, frequently-updated data structure between many workers — there is no equivalent of Go's atomic-read sharded engine from Course 1, because sharing memory is exactly what the model refuses to do. Where Go would shard a struct behind atomics for raw throughput, Erlang would shard the <em>problem</em> into more processes and accept the cost of a message copy at the boundary. For a system whose entire premise is "parts of it are broken right now," that trade is the right one: you are buying certainty about blast radius, and you are paying for it in copying overhead. Both are measured, in this course, rather than asserted.</p>
        </div>
        <h3>Architecture we are building toward</h3>
        <pre className="plain"><code>{"                         mesh_sup (one_for_one)\n                        /          |            \\\n              node_sup_1   registry_sup      dashboard_sup\n             (simple_one   (name -> pid,    (http listener,\n              _for_one)      process groups)  metrics collector)\n              /   |   \\\n          node  node  node  ...  (up to several thousand,\n           |     |     |          each a gen_server, each linked\n        mailbox mailbox mailbox   to node_sup_1, each able to\n                                   crash independently)\n\n  every arrow above is a supervision link: a child's death is\n  reported to its supervisor, which decides — by a declared,\n  static policy — whether to restart just that child, restart\n  every child alongside it, or give up and die itself, which\n  reports the death one level further up\n"}</code></pre>
        <p>Three architectural decisions, explained now because they shape every milestone that follows.</p>
        <p><strong>A node is a <code>gen_server</code></strong>, OTP's standard behaviour for "a process with state that answers requests" — you will write the raw <code>receive</code> loop version first in Milestone 3, by hand, before meeting <code>gen_server</code> in Milestone 5 as the same idea with the boilerplate handled for you and a long list of footguns removed.</p>
        <p><strong>Every node sits under a supervisor, and supervisors sit under supervisors.</strong> This is the <em>tree</em> in "supervision tree": <code>node_sup_1</code> is a <code>simple_one_for_one</code> supervisor, a template for spawning an unbounded, dynamic number of identical children — exactly what a few thousand interchangeable network nodes need — while <code>mesh_sup</code> at the root uses <code>one_for_one</code>, because the registry, the node pool and the dashboard are three genuinely different things that should not be restarted as a group just because one of them died.</p>
        <p><strong>The registry and the dashboard are processes too</strong>, supervised exactly like the nodes they serve. There is no special "infrastructure" tier that gets to skip the discipline the rest of the system follows — if the registry crashes, something notices and restarts it, the same as everything else.</p>
        ...
        <p>Why isolating failure is a design decision with a measurable cost, not a free lunch; how to describe a system's failure-response policy declaratively instead of scattering try/catch through it; how OTP's standard behaviours turn "a process that holds state and answers requests" from a hand-rolled loop into a few callback functions; what actually changes, in code, when a message-passing system stops being single-machine and becomes genuinely distributed; and why a network partition is not a bug you fix but a condition you are required to have an opinion about.</p>
        <hr />
        <h2><span className="num">Course 4 · Part 1</span>Install and first program</h2>
        <h3>What Erlang is</h3>
        <p>Erlang is a dynamically typed, functional, concurrent language built at Ericsson starting in 1986 to program telephone exchanges — hardware that was contractually required to have at most a few minutes of downtime a year. Two consequences follow directly from that origin and explain nearly everything the language does differently from a general-purpose scripting or systems language. First, the runtime (the <strong>BEAM</strong>, Erlang's virtual machine) was built around lightweight, isolated processes and message passing from the start, because a telephone exchange is naturally a huge number of mostly independent, concurrent calls. Second, the language and its standard library, together known as <strong>OTP</strong> (Open Telecom Platform), encode decades of "how do you build a system that must not go down" as reusable behaviours — <code>gen_server</code>, <code>supervisor</code>, and others — rather than leaving every team to reinvent them.</p>
        <p>Erlang is not "the old version of Elixir." Elixir is a newer language that compiles to the same BEAM bytecode and can call Erlang code directly (and vice versa); Erlang came first, by about 25 years, and this course uses it directly rather than through Elixir's syntax, because the OTP behaviours and the supervision-tree model — the actual subject of this course — are identical either way, and Erlang's syntax, once past the first hour of unfamiliarity, gets out of the way of that subject rather than adding a second one.</p>
        <h3>Installing</h3>
        <p>You want a recent OTP — this course was verified on <strong>OTP 25</strong> (Erlang/OTP 25, erts-13.1.5) — and, separately, <strong>rebar3</strong>, the standard build tool and package manager, roughly equivalent to Go's own toolchain or Ruby's Bundler.</p>
        <h5>Linux</h5>
        <pre className="plain"><code>{"sudo apt install erlang rebar3      # Debian/Ubuntu\nsudo dnf install erlang rebar3      # Fedora\n\nerl -eval 'io:format(\"~s~n\", [erlang:system_info(system_version)]), halt().' -noshell\n# Erlang/OTP 25 [erts-13.1.5] [source] [64-bit] [smp:N:N] [ds:N:N:N] [jit:ns]\n"}</code></pre>
        <h5>macOS</h5>
        <pre className="plain"><code>{"brew install erlang rebar3\n"}</code></pre>
        <h5>Windows</h5>
        <pre className="plain"><code>{":: the official installer from erlang.org, or:\nwinget install ErlangSolutions.Erlang\n:: rebar3 is a self-contained escript; download rebar3 and put it on PATH\n"}</code></pre>
        <p>WSL2 is worth it here, as it was for Perl: <code>observer</code> (Milestone 9) is a GUI tool that is simplest to run from a proper Linux environment, and the distributed-node work in Milestone 11 involves Unix-y networking assumptions that are easiest to reason about on Linux.</p>
        <h5>When you need a specific version</h5>
        <pre className="plain"><code>{"curl -fsSL https://raw.githubusercontent.com/kerl/kerl/master/kerl -o kerl\nchmod +x kerl\n./kerl build 25.3.2.9 25.3.2.9\n./kerl install 25.3.2.9 ~/erlangs/25.3.2.9\n. ~/erlangs/25.3.2.9/activate\n"}</code></pre>
        <p><code>kerl</code> is the Erlang-world equivalent of Perl's <code>perlbrew</code> or Go's own version management: it builds a specific OTP release from source into an isolated directory and gives you an <code>activate</code> script to bring it onto your <code>PATH</code>.</p>
        <h3>The shell is not optional</h3>
        <p>Every other course in this curriculum treats its language's REPL as a nice-to-have for quick experiments. Erlang's shell, <code>erl</code>, is closer to a permanent fixture of how you work: it can connect to a <em>running production system</em> and let you call functions, inspect process state, and even hot-load new code into it without stopping it — a capability the telephone-exchange origin story makes obvious in retrospect. You will use it constantly, not just at the start.</p>
        ...
        <p>That last line is not a typo in this document — it is the single most important fact about the language, arriving in the fourth line of the shell. <code>=</code> is not assignment. It is <strong>pattern matching</strong>: <code>X = 42</code> succeeds because <code>X</code> was unbound and binds it to <code>42</code>; <code>X = 42</code> again succeeds because <code>42</code> matches what <code>X</code> is already bound to; <code>X = 43</code> <em>fails</em>, loudly, with an exception, because <code>43</code> does not match <code>42</code>. Once a variable is bound in Erlang, it stays bound to that value for the rest of its scope — there is no reassignment, anywhere, ever. Section 2.1 below is entirely about what this buys you.</p>
        <h3>Project layout, with <code>rebar3</code></h3>
        ...
        <p><code>rebar3 new app</code> scaffolds an OTP <strong>application</strong> — not just a script, but a unit with a declared name, a supervision tree entry point, and a place for dependencies, tests and releases to live. That is deliberately more structure than Milestone 1 needs; the scaffold is there because by Milestone 6 you will need every part of it, and generating it once now means never having to retrofit it later.</p>
        <h3>Hello, mesh</h3>
        <pre><code>{"%% src/hello.erl\n-module(hello).\n-export([world/0]).\n\nworld() ->\n    io:format(\"hello, ~s~n\", [node()]).\n"}</code></pre>
        <pre className="plain"><code>{"$ erlc hello.erl\n$ erl -noshell -eval 'hello:world(), halt().'\nhello, nonode@nohost\n"}</code></pre>
        <h4>Every line, explained</h4>
        <ul>
          <li><strong><code>-module(hello).</code></strong> declares the module name, and it must match the filename (<code>hello.erl</code>) — the compiler enforces this, unlike Go's package-directory convention which is a strong suggestion rather than a rule.</li>
          <li><strong><code>-export([world/0]).</code></strong> lists which functions are visible outside this module, by name <em>and arity</em> — <code>world/0</code> is "the zero-argument function named world", a genuinely different, independently-exportable function from a hypothetical <code>world/1</code>. Nothing not listed here is callable from outside the module at all; there is no equivalent of Go's capitalisation convention, this is enforced by the compiler.</li>
          <li><strong><code>io:format("hello, ~s~n", [node()])</code></strong> calls the <code>format</code> function in the standard library's <code>io</code> module. <code>~s</code> is a format directive meaning "a string here"; <code>~n</code> is a portable newline; the second argument is always a <em>list</em> of values, one per directive, which is why a single argument still needs its own brackets.</li>
          <li><strong><code>node()</code></strong> returns the name of the current Erlang node — the runtime concept, not this course's "network node" concept, and the collision between those two uses of the word "node" is real and worth flagging now rather than mid-sentence in Milestone 11. <code>nonode@nohost</code> is what an Erlang VM is called when it was started without a name, which is the normal case until Milestone 11 gives every VM instance a real one.</li>
        </ul>
        <div className="warn">
          <h5>The two names for "node", and how to keep them apart</h5>
          <p>This course's simulated network is made of <strong>mesh nodes</strong> — ordinary Erlang processes, thousands of them, all living inside one BEAM instance for most of the course. Erlang's own runtime separately has a notion of a <strong>distributed node</strong> — one running BEAM instance, identified by a name like <code>mesh1@192.168.1.10</code>, that can connect to other BEAM instances over the network. Until Milestone 11 there is exactly one distributed node and thousands of mesh nodes running inside it; from Milestone 11 onward there are several distributed nodes, each still running many mesh nodes. The text says "process" whenever precision matters and "mesh node" for the simulated concept; "node" alone, unqualified, always means the Erlang runtime sense.</p>
        </div>
        <h3>Documentation, and getting help without leaving the shell</h3>
        ...
        <pre><code>{"%% test/basics_tests.erl\n-module(basics_tests).\n-include_lib(\"eunit/include/eunit.hrl\").\n\nclassify_test() ->\n    ?assertEqual(negative, basics:classify(-1)),\n    ?assertEqual(zero,     basics:classify(0)),\n    ?assertEqual(even,     basics:classify(4)),\n    ?assertEqual(odd,      basics:classify(7)).\n"}</code></pre>
        <pre className="plain"><code>{"$ rebar3 eunit\n===> Running EUnit tests...\n  basics_tests: classify_test...[0.001 s] ok\n  [done in 0.012 s]\n=======================================================\n  All 1 tests passed.\n"}</code></pre>
        <p><code>-include_lib("eunit/include/eunit.hrl")</code> pulls in the <code>?assertEqual</code> family of macros — yes, Erlang has a preprocessor and macros starting with <code>?</code>, inherited from its C heritage and used sparingly outside test code. A function whose name ends in <code>_test</code> (singular) is a single test; one ending in <code>_test_</code> (trailing underscore) is a <strong>test generator</strong>, returning a list of tests built programmatically — you will use both shapes throughout this course.</p>
        <h4>Checkpoint</h4>
        <ol>
          <li>What does <code>=</code> actually do in Erlang, and why does <code>X = 43</code> fail after <code>X = 42</code>?</li>
          <li>What two, independently meaningful things does a function's <em>arity</em> distinguish that a function's name alone does not?</li>
          <li>Name the two things a "node" can mean in this course, and which one <code>node()</code> the function returns.</li>
          <li>What is the difference between an EUnit test named <code>foo_test</code> and one named <code>foo_test_</code>?</li>
        </ol>
        <hr />
        <h2><span className="num">Course 4 · Part 2</span>Language crash course</h2>
        <h3>2.1 Pattern matching, and the rule with no equivalent in Go, Ruby or Perl</h3>
        <p>You met it above: <code>=</code> matches rather than assigns. The same mechanism — not a special case of it, the literal same operation — is how you pull values out of compound data, choose between function clauses, and receive messages. There is exactly one destructuring rule in the whole language, used everywhere.</p>
        <pre><code>{"1> {Status, Value} = {ok, 42}.\n{ok,42}\n2> Status.\nok\n3> [First | Rest] = [1,2,3,4].\n[1,2,3,4]\n4> First.\n1\n5> Rest.\n[2,3,4]\n6> {ok, 42} = {ok, 43}.\n** exception error: no match of right hand side value {ok,43}\n"}</code></pre>
        <p>Line 6 is not a corner case to remember — it is the mechanism working as designed. Matching a <em>literal</em> against a value, rather than a fresh variable against it, asserts that the value has exactly that shape. This is how Erlang code routinely reads: a function expects its argument to look like <code>{'{'}ok, Result{'}'}</code> or its message to look like <code>{'{'}From, Ref, Request{'}'}</code>, and if it does not, the match fails and the process crashes — which, per this course's whole premise, is the correct response, not a bug to prevent.</p>
        <p>Compare this to how a typical language handles the same job:</p>
        <div className="cmp">
          <h5>A typical language vs. Erlang</h5>
          <p>In Python or Ruby you would write <code>status, value = pair</code> and then a separate <code>if status != "ok": raise ...</code> to assert the shape you expected. Erlang's match <em>is</em> the assertion — there is no separate step, because the destructuring and the shape check are the same operation. The cost is that a bound variable can never be reused for something else in the same scope; the benefit is that "this data has the shape I assumed" is enforced at the exact point you assume it, not several lines later when something built from the wrong assumption finally breaks.</p>
        </div>
        <h4>Exercise 2.1</h4>
        <div className="exercise">
          <p>In the shell, bind <code>Point = {'{'}3, 4{'}'}</code>. Write a pattern match that extracts both coordinates into <code>X</code> and <code>Y</code> in one line, then write an expression using only pattern matching (no <code>if</code>, no function call) that succeeds if and only if the point is exactly the origin, <code>{'{'}0, 0{'}'}</code>.</p>
          <details>
            <summary>Solution</summary>
            <pre><code>{"1> Point = {3, 4}.\n{3,4}\n2> {X, Y} = Point.\n{3,4}\n3> {0, 0} = Point.\n** exception error: no match of right hand side value {3,4}\n"}</code></pre>
            <p>The third line is the answer to the second half: it succeeds silently for the origin and raises for anything else, which <em>is</em> "succeeds if and only if". Wrapping it in <code>catch {'{'}0,0{'}'} = Point</code> converts the exception into an ordinary value if you need the result rather than the side effect of not crashing.</p>
          </details>
        </div>
        <h3>2.2 Atoms, tuples and the convention that runs through everything</h3>
        <p>An <strong>atom</strong> is a name that is its own value — <code>ok</code>, <code>error</code>, <code>undefined</code>, <code>mesh_node</code> — comparable in spirit to a Ruby symbol, with no equivalent that is quite as central in Go or Perl. Atoms are used everywhere a smaller language would reach for a string constant or a small integer enum, and OTP's standard library leans on one convention hard enough that you should adopt it immediately: a function that can fail returns <code>{'{'}ok, Result{'}'}</code> on success and <code>{'{'}error, Reason{'}'}</code> on failure, as a two-element tuple, rather than throwing.</p>
        <pre><code>{"1> file:read_file(\"does_not_exist.txt\").\n{error,enoent}\n2> file:read_file(\"/etc/hostname\").\n{ok,<<\"my-machine\\n\">>}\n"}</code></pre>
        ...
        <p>Both compute the same thing and both are used in this course — <code>fact/1</code>'s shape is completely fine for small, bounded recursion, and its accumulator-passing sibling is what you reach for once the recursion depth is genuinely unbounded, which for a mesh of several thousand nodes, it routinely will be.</p>
        <div className="cmp">
          <h5>A typical language vs. Erlang</h5>
          <p>Go and Ruby both have <code>for</code> loops with mutable loop variables; writing the equivalent accumulation is a variable you reassign each iteration. Erlang cannot reassign a variable at all (Section 2.1), so the accumulator has to be threaded through as an extra function argument instead — which looks like more ceremony for a five-line function and stops looking that way the moment the accumulator needs to be something more interesting than a running total, because it is then just an ordinary function parameter with an ordinary type, not a mutable variable whose type you have to keep straight across dozens of loop iterations by eye.</p>
        </div>
        <h4>Exercise 2.3</h4>
        <div className="exercise">
          <p>Write <code>my_reverse/1</code>, reversing a list, tail-recursively (accumulator-passing), without using the built-in <code>lists:reverse/1</code>.</p>
          <details>
            <summary>Solution</summary>
            <pre><code>{"my_reverse(List) -> my_reverse(List, []).\nmy_reverse([], Acc) -> Acc;\nmy_reverse([H | T], Acc) -> my_reverse(T, [H | Acc]).\n"}</code></pre>
            <p>Each step moves the head of the input onto the front of the accumulator, which is exactly reversal happening one element at a time — verified: <code>my_reverse([1,2,3])</code> gives <code>[3,2,1]</code>.</p>
          </details>
        </div>
        <h3>2.4 Maps</h3>
        ...
        <pre><code>{"1> Pid = spawn(echo, loop, []).\n<0.94.0>\n2> Ref = make_ref().\n#Ref<0.123.456.789>\n3> Pid ! {self(), Ref, hello}.\n{<0.85.0>,#Ref<0.123.456.789>,hello}\n4> flush().\nShell got {#Ref<0.123.456.789>,{echo,hello}}\nok\n"}</code></pre>
        <p><strong><code>spawn/3</code></strong> starts a new, independent process running <code>echo:loop()</code> and returns its <strong>process identifier</strong> (a <code>pid</code>) immediately, without waiting for it to do anything — spawning is not a blocking operation and there is no thread limit to worry about hitting. <strong><code>!</code></strong> (pronounced "bang") sends a message: it copies the term on its right into the target process's <strong>mailbox</strong> — an unbounded, per-process FIFO queue that only that process reads from — and returns immediately, whether or not anyone is listening. <strong><code>receive</code></strong> blocks the calling process until a message matching one of its patterns arrives in its own mailbox, then runs the matching clause; a bare <code>loop()</code> as the tail of each clause is exactly the tail-recursion from Section 2.3, and it is what makes this an ongoing server rather than a one-shot responder.</p>
        <p>The reply pattern — <code>{'{'}From, Ref, Msg{'}'}</code> in, <code>{'{'}Ref, Reply{'}'}</code> back — is not a special language feature, it is a convention, and it is worth understanding why the <code>Ref</code> is there at all rather than replying to <code>From</code> directly. <code>make_ref/0</code> creates a value guaranteed unique for the lifetime of the runtime; including it lets the caller distinguish a reply to <em>this specific request</em> from some unrelated message that happens to also be addressed to it, which matters the moment a process can have several requests in flight at once. <code>gen_server</code>, met in Milestone 5, does exactly this under the hood, automatically.</p>
        <h4>Selective receive</h4>
        <pre><code>{"receive\n    {Ref, Reply} -> Reply     %% only matches a message tagged with THIS Ref\nafter 1000 ->\n    timeout\nend\n"}</code></pre>
        <p><code>receive</code> does not take the next message in the mailbox unconditionally — it scans for the first message that matches one of its clauses, leaving anything that does not match sitting in the mailbox for a later <code>receive</code> to find. This is <strong>selective receive</strong>, and the <code>Ref</code> convention above is precisely what makes it possible to say "wait specifically for the reply to this request" in a process that might have other, unrelated messages arriving in the meantime. <code>after 1000 -{'>'}</code> bounds the wait to one second, returning <code>timeout</code> if nothing matching arrives in time — without it, a reply that never comes blocks the caller forever.</p>
        <div className="warn">
        ...
        <table className="grid">
          <tbody>
            ...
            <tr>
              <td>Direction</td>
              <td>bidirectional — either side dying affects the other</td>
              <td>one-directional — only the watcher is notified</td>
            </tr>
            <tr>
              <td>Default effect of the other side dying</td>
              <td>you die too, propagating the exit signal onward, unless you set <code>trap_exit</code></td>
              <td>you receive a <code>{'{'}'DOWN', ...{'}'}</code> message; you are never killed by it</td>
            </tr>
            <tr>
              <td>Used for</td>
              <td>supervision — a supervisor and its children are always linked</td>
              <td>observation without ownership — the registry watching nodes it does not supervise</td>
            </tr>
          </tbody>
        </table>
        ...
        <p><code>process_flag(trap_exit, true)</code> is what turns a link from "their death kills me too" into "their death sends me a plain <code>{'{'}'EXIT', Pid, Reason{'}'}</code> message instead" — this is exactly the flag every OTP supervisor sets, because a supervisor's entire job is to survive its children's deaths and react to them, which is the opposite of the default linked behaviour. <code>spawn_monitor/1</code> both spawns and monitors in one call, convenient for exactly the "watch this thing, but I am not responsible for it" relationship the registry will have with nodes in Milestone 7.</p>
        <h3>2.9 Errors: <code>try</code>/<code>catch</code>, and when not to use it</h3>
        ...
        <p>Compare the raw <code>receive</code> loop from Section 2.7 to this: the loop itself, the reply-matching convention, the timeout handling — all of it is gone, replaced by three small functions that only describe <em>what should happen</em>, never <em>how the message actually gets there</em>. That is the entire value proposition of an OTP behaviour, and Milestone 5 is the moment you stop hand-writing the "how" and start trusting a library that has handled it correctly for four decades.</p>
        <h3>2.11 Testing with EUnit, properly</h3>
        <pre><code>{"-module(mesh_node_tests).\n-include_lib(\"eunit/include/eunit.hrl\").\n\n%% a fixture: setup, the tests that need it, then teardown — for when a\n%% test needs a running process rather than a pure function\nspawn_and_stop_test_() ->\n    {setup,\n     fun() -> {ok, Pid} = mesh_node:start_link(#{id => 1}), Pid end,\n     fun(Pid) -> mesh_node:stop(Pid) end,\n     fun(Pid) ->\n         [?_assert(is_process_alive(Pid)),\n          ?_assertEqual(alive, mesh_node:status(Pid))]\n     end}.\n"}</code></pre>
        <p>The <code>{'{'}setup, Setup, Cleanup, Tests{'}'}</code> tuple is EUnit's fixture shape, roughly parallel to Go's table-driven tests with a shared <code>TestMain</code>, or RSpec's <code>before</code>/<code>after</code> — and it is the shape nearly every test in Milestones 3 onward will use, because nearly every test needs a real, running process to test against.</p>
        <h4>Part 2 checkpoint</h4>
        <ol>
          <li>Why is a guard restricted to a small set of side-effect-free operations, rather than allowed to call any function?</li>
          <li>What is the actual difference between a linked process and a monitored one, and which relationship does a supervisor have with its children?</li>
          <li>Why does an unmatched message sitting forever in a mailbox get worse over time, specifically?</li>
          <li>Name the three ways Erlang signals something exceptional, and which one this course's philosophy argues you should reach for least.</li>
          <li>What does a behaviour's callback module <em>not</em> have to implement, compared to the raw <code>receive</code> loop version of the same server?</li>
        </ol>
        <h3>What is next</h3>
        <p>
          <img className="mascot-left" src={img2.src} alt="The Mewlang cat, typing at a laptop" width="120" loading="lazy" />
          Milestone 1 turns the shell experiments above into a real, compiled, tested <code>rebar3</code> project. Milestone 2 builds the pure data-handling functions the rest of the node will need. Milestone 3 is where a mesh node becomes a real, running process for the first time — everything from Section 2.7 onward, applied.
        </p>
        <footer className="end">
          <p>Instalment 16 of the five-course curriculum. Next: Erlang Milestones 1–4, where the shell experiments above become a real project, and a node becomes a process that can crash on purpose.</p>
        </footer>
         <Link className="button" href="/erlang-course/milestones/1-4/">Continue</Link> 
      </div>
    </div>
  );
}
